class AppPager {
    constructor(currItem, newContent) {
        this.currPageItem = currItem;
        this.currPageContent = newContent;
    }

    setNewContent() {
        document.getElementById("changeMe").innerHTML = this.currPageContent;
    }
} 
        ...
        <div id="menuLinks" class="container">
            ...
            <nav>
                <ul>
                    ...
                    ${isLoggedIn ? '<li class="nav nav-item"><a class="nav-link" id="logout4">Log out</a></li>' : '<li class="nav nav-item"><a class="nav-link" id="login4" href="src/login/sign-in.html">Login / Register</a></li>'}
                </ul>
            </nav>
        </div>
    `;
...
function signInAndBegin(){
    ...
    signInButton.addEventListener("click", () =>{
        window.location.href = "src/login/sign-in.html";
    });
}
